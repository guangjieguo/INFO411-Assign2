...
# ╔═╡ 83a34dee-36d5-4707-8cc5-9bafb6815db3
using DataFrames, CSV, MLJ, Random, MLJLinearModels,  Statistics, StatsBase, LinearAlgebra , NearestNeighbors, CategoricalArrays, StatsPlots, NearestNeighborModels, PlutoUI, HypertextLiteral

# ╔═╡ 71793d70-68be-11ee-37c6-d175155bfde4
md"
#### Dashboard for visualization of influence of the number of neighbors considered in KNN modelling
"

# ╔═╡ 09fa1d53-9816-4ba2-9f38-088bcc3148e8
begin
	data_trc = CSV.read("cleveland_imputed.csv", DataFrame)
	category_list = []
	for i in 1:303
		if data_trc[i, "num"] == 0
...
# ╔═╡ 606b4104-2dfd-4114-a91d-86cfda94cff5
begin
    Random.seed!(3)
    tr_inds, te_inds = MLJ.partition(1:nrows(X), 0.8, shuffle=true)
end

# ╔═╡ 107e00a3-1018-46dd-8426-4df622e587e6
begin
	stand1 = Standardizer()
...
# ╔═╡ aa5da405-9d17-4fb4-8f5f-91b3b10dbd58
MLJ.accuracy(StatsBase.mode.(y_pred), y[te_inds])

# ╔═╡ 3fb2e056-2724-4bdf-bf6e-bc0c38cc0fd4
begin
	slider = @bind num PlutoUI.combine() do Child
		md"""
		##### This is to show the performance of KNN with different number of neighboors considered

		###### The number of neighboors considered is  
		
		$(
			Child(Slider(1:1:40; show_value=true))
		).

		###### Comment: It can be seen from the dashboard that the performance of KNN improves significantly by increasing K when K is less than 10. However, when K is greater than ten, there is no significant change in the accuracy as K increases.
		"""
	end
	
end

# ╔═╡ 581632ac-37ef-4466-be29-7ded6d534224
begin
	...
	function plotk(β)
		...
		SP = StatsPlots.violin(["$β"], per_fold_knn, ylims=(0.5,1), label=nothing)
	end
end 

# ╔═╡ e3178ea5-712a-4355-aa0a-26d2f3ab7fac
begin
	@info PlutoRunner.currently_running_cell_id
	PlutoUI.ExperimentalLayout.hbox([slider, plotk(num[1])])
end

# ╔═╡ 9a3ce9d6-8171-4149-af59-1f6f7a862142
notebook = PlutoRunner.notebook_id[] |> string

# ╔═╡ 7d2e3c54-3597-434c-9849-302e2d50d088
celllist=["e3178ea5-712a-4355-aa0a-26d2f3ab7fac"]

# ╔═╡ 238d2b29-cb66-4ba1-a626-83286f567feb
dash_final_url="http://localhost:1234/edit?" * "id=$notebook&" * join(["isolated_cell_id=$cell" for cell in celllist], "&")

# ╔═╡ 34c65fab-204b-4f64-9ca8-2c8c8a624cde
@htl("""
<a href="$dash_final_url" style="font_size=20">Click here for the knn Dashboard</a>
""")
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "a9427f9c546f5b354046da9eec32957f26b59f0a"
